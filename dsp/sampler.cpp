    ...
    void process(const float* input, float* output, int numSamples)
    {
        ...
        while (numSamplesProcessed < numSamples)
        {
            ...
            size_t samplesToProcess = std::min(
                static_cast<size_t>(numSamples - numSamplesProcessed),
            ...
            if (inputBufferWasEmpty)
            {
                ...
                std::fill(inputSegmentData + fftSize_, inputSegmentData + fftSize_ * 2, 0.0f);
            ...
            if (inputDataPos_ == blockSize_)
            {
                ...
                currentSegment_ = (currentSegment_ > 0) ?
                    (currentSegment_ - 1) : (numInputSegments_ - 1);
            }
